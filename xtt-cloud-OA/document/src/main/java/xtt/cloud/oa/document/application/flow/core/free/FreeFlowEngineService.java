package xtt.cloud.oa.document.application.flow.core.free;
...
import xtt.cloud.oa.document.domain.entity.flow.definition.free.FlowActionRule;
...
public class FreeFlowEngineService {
    ...
    private final ApproverScopeRepository approverScopeRepository;
    
    private final FreeFlowNodeInstanceRepository freeFlowNodeInstanceRepository;
    ...
    public FreeFlowEngineService(
            FlowActionRepository flowActionRepository,
            FlowActionRuleRepository flowActionRuleRepository,
            ApproverScopeRepository approverScopeRepository,
            FreeFlowNodeInstanceRepository freeFlowNodeInstanceRepository,
        ...
        this.approverScopeRepository = approverScopeRepository;
        this.freeFlowNodeInstanceRepository = freeFlowNodeInstanceRepository;
    ...
    public ApproverScope getApproverScope(Long actionId) {
        ...
        try {
            ApproverScope scope = approverScopeRepository.findByActionId(actionId);
    ...
    public FlowNodeInstance executeAction(
        ...
        try {
            // 1. 验证当前节点
            FlowNodeInstance currentNode = flowNodeInstanceRepository.findById(currentNodeInstanceId);
            if (currentNode == null) {
            ...
            // 2. 获取流程实例和文档信息
            FlowInstance flowInstance = flowInstanceRepository.findById(currentNode.getFlowInstanceId());
            ...
            // 3. 获取节点定义
            FlowNode nodeDef = flowNodeRepository.findById(currentNode.getNodeId());
            if (nodeDef == null) {
            ...
            if (action.getActionType() == FlowAction.TYPE_RETURN) {
                return handleReturnAction(currentNode, flowInstance, document, nodeDef, comment, operatorId);
            }
            ...
            for (Long approverId : approverIds) {
                ...
                freeFlowNodeInstanceRepository.save(freeNodeInstance);
            ...
            flowNodeInstanceRepository.update(currentNode);
    ...
    private List<FlowActionRule> matchActionRules(
            ...
            Long deptId) {
        return flowActionRuleRepository.matchRules(documentStatus, userRoles, deptId);
    }
    ...
    private FlowNodeInstance handleReturnAction(
        ...
        if (previousNode == null) {
            // 没有上一个节点，退回给发起人
            returnToUserId = document.getCreatorId();
        ...
        // 创建返回节点实例
        // TODO: 在 Repository 中添加根据类型查询的方法
        FlowAction returnAction = null; // flowActionRepository.findByActionType(FlowAction.TYPE_RETURN);
        
        if (returnAction == null) {
            // TODO: 实现返回动作查询逻辑
            throw new BusinessException("未找到返回动作定义，功能待实现");
        }
        
        // 以下代码在 returnAction 不为 null 时执行（当前为 dead code，待实现功能后启用）
        ...
        flowNodeInstanceRepository.save(returnNodeInstance);
    ...
    private FlowNodeInstance findPreviousNodeInstance(Long flowInstanceId, Long currentNodeInstanceId) {
        ...
        List<FlowNodeInstance> allInstances = flowNodeInstanceRepository.findByFlowInstanceId(flowInstanceId);
        List<FlowNodeInstance> previousInstances = allInstances.stream()
            .filter(instance -> instance.getCreatedAt() != null && 
                    currentNode.getCreatedAt() != null &&
                    instance.getCreatedAt().isBefore(currentNode.getCreatedAt()))
