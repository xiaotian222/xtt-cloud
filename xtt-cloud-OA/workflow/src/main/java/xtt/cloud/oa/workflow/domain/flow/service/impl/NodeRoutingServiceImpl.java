...
public class NodeRoutingServiceImpl implements NodeRoutingService {
    ...
    public List<Long> getNextNodeIds(Long currentNodeId, Long flowDefId, Map<String, Object> processVariables) {
        ...
        java.util.Optional<xtt.cloud.oa.workflow.domain.flow.model.entity.FlowNode> currentNodeOpt =
                flowNodeRepository.findById(FlowNodeId.of(currentNodeId));
        ...
        List<xtt.cloud.oa.workflow.domain.flow.model.entity.FlowNode> allNodes =
                flowNodeRepository.findByFlowDefId(FlowDefinitionId.of(flowDefId));
    ...
    private List<Long> evaluateConditionalNodes(
        ...
        for (Long nodeId : candidateNodeIds) {
            java.util.Optional<xtt.cloud.oa.workflow.domain.flow.model.entity.FlowNode> candidateNodeOpt =
                    flowNodeRepository.findById(FlowNodeId.of(nodeId));
    ...
    @Override
    public void moveToNextNode(FlowInstance flowInstance, FlowNode currentNodeDef, FlowNodeInstance currentNode) {
        ...
        List<Long> nextNodeIds = getNextNodeIds(
                currentNodeDef.getId(),
        ...
        for (Long nextNodeId : nextNodeIds) {
            if (isConvergenceNode(nextNodeId, flowInstance.getFlowDefId())) {
                // 如果是汇聚节点，检查是否可以汇聚
                if (!canConverge(nextNodeId,
                        flowInstance.getId() != null ? flowInstance.getId().getValue() : null)) {
            ...
            // 4. 检查是否应该跳过节点
            if (shouldSkipNode(nextNodeId, flowInstance.getFlowDefId(), processVariables)) {
                // 创建已跳过的节点实例
...
//                    processVariables);
            ...
            if (approvers.isEmpty()) {
                throw new IllegalStateException("节点 " + nextNode.getNodeName() + " 无法分配审批人");
            }
            ...
            log.info("节点实例创建成功，节点名称: {}, 创建了 {} 个实例", nextNode.getNodeName(), approvers.size());
        }
    ...
    public void rollbackToNode(FlowInstance flowInstance, Long targetNodeId, Long operatorId, String reason) {
        ...
        // 1. 验证目标节点是否存在
        FlowNode targetNode = flowNodeRepository.findById(FlowNodeId.of(targetNodeId))
